...
const socialLinks = [
  
  {
    ...
    subText: "On LinkedIn",
    icon: Linkedin,
    url: "https://www.linkedin.com/in/vivekreddy06/",
    color: "#0A66C2",
...

  {
    name: "GitHub",
    displayName: "Github",
    subText: "@vivekreddy9036",
    icon: Github,
    url: "https://github.com/vivekreddy9036",
    color: "#ffffff",
...

  {
    ...
    subText: "@vivekreddy006",
    icon: ({ className, ...props }) => (
      <img src={TryhackmeImg} alt="TryHackMe" className={className + " rounded-sm"} {...props} />
    ),
    url: "https://tryhackme.com/p/vivekreddy006",
    gradient: "from-[#2EC866] to-[#0AA05A]",
  },
  
  {
    name: "Instagram",
    displayName: "Instagram",
    subText: "@vivekreddy_006",
    icon: Instagram,
    url: "https://www.instagram.com/vivekreddy_006/?hl=en",
    color: "#E4405F",
...
const SocialLinks = () => {
  ...
  const instagram = otherLinks.find((l) => l.name === "Instagram");
  const tryhackme = otherLinks.find((l) => l.name === "TryHackMe");
  const github = otherLinks.find((l) => l.name === "GitHub");
  const tiktok = otherLinks.find((l) => l.name === "TikTok");
  ...
  return (
    <div className="w-full bg-gradient-to-br from-white/10 to-white/5 rounded-2xl p-6 py-8 backdrop-blur-xl">
      ...
      <div className="flex flex-col gap-4">
        ...
        {/* Second Row - GitHub & TryHackMe */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[github, tryhackme].filter(Boolean).map((link, index) => (
            <a
              key={link.name}
              href={link.url}
              target="_blank"
        ...
        {/* Third Row - Instagram (keeps same sizing as other rows; placeholder makes room for a 4th) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[instagram, null].map((link, index) => {
            if (!link) {
              // Invisible placeholder to keep the grid cell sizing consistent and ready for a 4th card
              return (
                <div
                  key={`placeholder-${index}`}
                  className="p-4 rounded-xl bg-transparent border border-transparent opacity-0 pointer-events-none"
                />
              );
            }
